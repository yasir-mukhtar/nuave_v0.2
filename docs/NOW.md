...
dispatch remains v2. **G2 is prepared and offline-verified** on
`codex/spec008-g2-evaluation-freeze` (draft PR pending review): a dormant v3
rich/simple writer prototype, bounded finalizer/fallback, the frozen
evaluation packet
([`G2_EVALUATION_PACKET.md`](../specs/008-recommendation-eligible-question-generation/G2_EVALUATION_PACKET.md)),
and executable §8.3 decision-rule tests. It awaits independent acceptance; it
is not an empirical quality pass, and provider feasibility is unproven. The
...
next action is G2 independent review, then a separately authorized G2P pilot.
