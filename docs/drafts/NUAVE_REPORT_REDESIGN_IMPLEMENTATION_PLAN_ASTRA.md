...
- **G4 / settled:** the answers-only state has copy but no print/JSON save path. Founder confirmed on 2026-09-22 (eighth settled decision).
...
|---|---|---|
| 0. Record decisions and approve the spec | Review v3, then create one numbered spec. Record all eight founder decisions. Amend `docs/VOICE.md` at that stage to settle **Bisnis Anda direkomendasikan di Y dari 10 pertanyaan**; retain **Bisnis Anda muncul di X dari 10 pertanyaan** and **Download PDF**. Record the `ReportToolbar` default-label change as a correction to code, not a new voice choice. Explicitly amend Spec 010's **“Observation/report failure”** row for the approved ten-answer recovery case whenever the final gate fails (findings empty, actions empty, or both): answers plus unfinished-analysis notice only, no surviving analysis exposed, existing retry/cost rules, no completed-observation rerun or partial report. **G4, founder-confirmed 2026-09-22:** in that state the customer can copy answers but cannot print or download them; the retry control is the intended path. No PDF or JSON in the answers-only state. Record same-order/single-tree print and reconcile `AUDIT.md` / journey 06. Preserve the gate for explicit approval of `react-markdown` and `remark-gfm` with exact compatible pins; review the concrete non-corrective template contract. | One approved spec, exact labels and dependency pins, required canonical/Spec 010 amendments, fixtures/rubric, and separate A/B1/B2 allowlists. Founder decisions are recorded, not reopened. This task performs none of those other file edits or installs. |
...
- **Next smallest action: promotion to one numbered specification. All eight founder decisions are settled.**
