# Nuave Report Redesign — Investigation, Design Strategy, and Implementation Plan — Astra (v3)

> Status: **Draft v3 for independent review**
> Author: Astra
> Date: 2026-09-22
...
| Recovery | When repair removes every finding and action, the customer may read all ten retained model answers with a clear unfinished-analysis notice. No findings/actions or partial report are delivered. | Amend Spec 010's “Observation/report failure” row explicitly; implement the bounded exception in PR B2. |
...
**All ten full answers start open.** There is no primary “read full answer” action and no preview selector. If disclosure is retained, allow multiple answers to stay open and provide secondary `Tutup semua jawaban` / `Buka semua jawaban` controls. Closing an answer must leave its exact question and result labels visible. Do not nest source links or copy controls inside the disclosure trigger. Reading the default report requires no expansion interaction. If optional collapse remains, the single answer body must stay mounted so print CSS can expose it; section 6.8 defines the gate.
...
The final useful-report gate still requires at least one supported finding and action. If only one required section survives repair, do not deliver that incomplete report or expose its surviving analysis; retain the existing report-failure/retry state. The approved answers-only case is specifically the both-empty result. A supported maintenance/investigation action may satisfy the minimum; filler may not. No automatic observation rerun or extra synthesis pass is introduced.
...
All answers are open by default. If secondary collapse controls are retained, keep their content mounted and make print CSS expose it irrespective of screen state, including through the browser's own print command. Remove height/overflow clipping in print. If the chosen primitive cannot satisfy this with the single tree and CSS, omit the optional collapse controls from PR A and keep answers always visible; do not add a second print tree or a JavaScript regeneration path. Hide copy/toggle controls in print while retaining the question, answer, sources, and provenance.
...
**Founder answer 1 / F1: revert the direct-ten toolbar label to the settled `Download PDF`.** In PR A, pass `pdfLabel="Download PDF"` through the existing `ReportToolbar` prop at the call site outside the protected header/brief region. This corrects a pre-existing code-versus-`VOICE.md` discrepancy; it is not a new voice decision. Keep the existing `window.print()` mechanism and ordinary browser save-as-PDF flow. Do not introduce a hosted PDF service or promise an automatically downloaded file. Update the exact-label browser test locator in `tests/e2e/new-intake-glm.spec.ts`, retaining its existing print/no-repeat assertions.
...
|---|---|---|
| 0. Record decisions and approve the spec | Review v3, then create one numbered spec. Record all seven founder decisions. Amend `docs/VOICE.md` at that stage to settle **Bisnis Anda direkomendasikan di Y dari 10 pertanyaan**; retain **Bisnis Anda muncul di X dari 10 pertanyaan** and **Download PDF**. Record the PR A toolbar override as a correction to code, not a new voice choice. Explicitly amend Spec 010's **“Observation/report failure”** row for the approved ten-answer, both-empty-analysis recovery case: answers plus unfinished-analysis notice only, existing retry/cost rules, no completed-observation rerun or partial report. Record same-order/single-tree print and reconcile `AUDIT.md` / journey 06. Preserve the gate for explicit approval of `react-markdown` and `remark-gfm` with exact compatible pins; review the concrete non-corrective template contract. | One approved spec, exact labels and dependency pins, required canonical/Spec 010 amendments, fixtures/rubric, and separate A/B1/B2 allowlists. Founder decisions are recorded, not reopened. This task performs none of those other file edits or installs. |
...
| 2. Make the report readable — A | Exact settled count labels with quiet completion; fixed layer labels; no untested tiles, arbitrary finding quote, or repeated action. Scoped body CSS; toolbar `pdfLabel="Download PDF"`; update the affected exact-label browser locator. Keep stored content and backend limits/recovery unchanged. | Desktop/mobile hierarchy, exact labels, evidence links/copy, and compatible historical renderer pass. Header/date limits and B2 usefulness work are explicitly deferred. |
...
| AC08 | `Download PDF` invokes the existing browser print path. PDF prints the single screen tree in screen order, with all ten exact questions/full answers appearing once, no appendix or duplicate direct-ten print tree. If collapse controls remain, print is complete even after collapse through both toolbar and browser print commands. Inspect every page: no orphan headings, short blocks together when practical, long answers/tables/URLs reflow. Screen/PDF/JSON retain the same evidence. |
...
| AC10 | At 390px and 1440px, questions, full answers, controls, and action hierarchy are usable; also check 320px reflow and 200% zoom. Keyboard users can toggle optional disclosures and follow references with visible focus. The new direct-ten view contains one answer tree and one set of anchors; there is no duplicate print tree to hide from screen readers. |
...
| Repair removes every finding and action; ten usable observations remain | B2 shows the answers-only reader and unfinished-analysis notice under the amended Spec 010 row, with no rejected classifications, conclusions, findings/actions, report-ready controls, or new calls. A/B1 preserve baseline recovery. |
| Only one required section survives, or the ten-answer set is incomplete/unsafe | B2 delivers no partial report and does not broaden the both-empty exception; the applicable existing failure/retry or data-handling path remains in force. |
...
| Single-tree browser PDF with short/multi-page answers and optional manually closed answers | PDF section order matches the screen, each question/full answer appears once, CSS exposes closed-but-mounted content if collapse is retained, and every page passes heading/grouping/reflow checks; no duplicate tree or appendix. |
| Direct-ten toolbar and exact-label browser selector | The visible action is `Download PDF`, backed by the existing print callback; the test locator uses the settled wording without weakening no-repeat assertions. |
...
7. Are same-order/single-tree print, inert answer URLs, and the amended answers-only recovery state concretely testable, including manual collapse and absence of rejected report analysis?
...
- **Next smallest action: independent review of v3, then promotion to one numbered specification.**
