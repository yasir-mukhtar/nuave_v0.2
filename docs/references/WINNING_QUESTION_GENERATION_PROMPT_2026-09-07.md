# Founder-accepted question-generation prompt — recovered 2026-09-16

Status: preserved source for the accepted generation approach; not a new implementation plan or a claim of production activation.

Source: [Winning Prompt Generation Approach](chatgpt-conversation://6a9e8d11-ba2c-83ec-96f3-10887d089ba0), assistant document `58314`, generated 2026-09-07. The founder subsequently wrote: “Experiment successful! It consistently produce recommendation-eligible intent across different models.” The founder asked the implementation-planning agent to preserve this approach without degrading it, and identified this conversation again on 2026-09-16 as the winning reference.

The prompt below is copied exactly from the complete retrieved document body, from its task heading through the sample-brand placeholder. It is not a reconstruction from R5. The conversation contains the instruction, examples, and founder acceptance; it does not contain separate attributed GLM-5.3/GLM-5.3 Flash response captures. Those model-specific results remain founder-reported.

## Current reading instructions

- Preserve the natural consumer-choice approach and its examples as the quality reference.
- The [2026-09-16 founder amendment](../DECISION_LOG.md#2026-09-16--natural-questions-may-address-the-same-need) supersedes any interpretation of this source's diversity guidance as a quota. The same need can recur naturally; do not invent different needs to fill a pack.
- The [2026-09-18 approved delivery decision](../DECISION_LOG.md#2026-09-18--approve-the-bounded-recommendation-eligible-delivery) removes fixed functional slots and adopts ten directly generated unnamed questions with human editing/approval. [Spec 009](../../specs/009-recommendation-eligible-audit/SPEC.md) is the approved local implementation contract. The source's twelve-candidate experiment remains unchanged below; twelve→ten is an explicit runtime adaptation, not permission to rewrite its reasoning or restore 6/4. Paid calls remain separately authorized.
- Conversely, compliance with those implementation rules does not establish naturalness or fidelity to this source. Surface any conflict explicitly; do not force unnatural wording to satisfy a slot.
- The founder's latest cafe/laptop wording and dental-question rejection are recorded in the decision log. These are exact-text feedback, not universal templates or numerical scores.
- GLM-5.3 Flash integration is preserved separately in [draft PR #47](https://github.com/yasir-mukhtar/nuave_v0.2/pull/47). That PR changes the provider while retaining the old writer instruction; merging it alone would not install this prompt.
- No provider call, implementation, commit, push, merge, or deployment is authorized by this source recovery.

## Exact recovered prompt

<!-- BEGIN EXACT RECOVERED PROMPT -->
# Task: Generate natural consumer questions with genuine business-recommendation opportunity
