...
describe("Bab 2-3 wiring map", () => {
  it("exports exactly the four owned screens, all valid screen ids", () => {
    expect(Object.keys(BAB2_SCREENS).sort()).toEqual(
      ["s-competitors", "s-facts", "s-market", "s-review"].sort(),
    );
...
describe("s-competitors (F1 rich)", () => {
  ...
  it("thin fixture retains the same correction grammar", () => {
    const thin = renderBab2("s-competitors", F2);
    expect(thin).toContain("Tambah bisnis lain");
  });
...
describe("s-review readback (F1 rich)", () => {
  ...
  it("links every row via whole-row chevron buttons with valid targets", () => {
    ...
    expect(html).toContain("tabler-icon-chevron-right");
  });
  ...
  it("adds the F3 advisory conflict row without blocking wording", () => {
    ...
    expect(conflict).toContain("data-correction-target");
  });
...
describe("question boundary ownership", () => {
  it("keeps fixture structure available while the controlled question module owns review", () => {
    expect(BAB2_SCREENS["s-questions"]).toBeUndefined();
    expect(deriveQuestionSlots(F1)).toHaveLength(10);
    expect(
      deriveQuestionSlots(F1).filter((slot) => slot.unbranded),
    ).toHaveLength(6);
  });
  it("keeps display controls free of network calls", () => {
    const source = readFileSync(
      new URL("./screens-bab2.tsx", import.meta.url),
      "utf8",
    );
    expect(source).not.toMatch(/fetch\(|XMLHttpRequest/g);
  });
});
...
describe("Bab 2-3 house rules", () => {
  const all = (["s-market", "s-competitors", "s-facts", "s-review"] as const)
  ...
  it("uses the shared accessible UI primitives", () => {
    expect(all).toContain('data-slot="checkbox"');
    expect(all).toContain('data-slot="button"');
  });
});
...
describe("shell pairing (transitions + terminal)", () => {
  ...
  it("starts at preparation even if an isolated-screen driver is supplied", () => {
    const html = renderToStaticMarkup(
      createElement(IntakeJourney, { initialScreenId: "s-review" }),
    );
