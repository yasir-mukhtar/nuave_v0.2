...
function isAllowedImportSpecifier(specifier: string): boolean {
  return (
    ...
    specifier.startsWith("@/lib/audit/") ||
...
describe("new intake isolation guard", () => {
  ...
  it("only imports the new module, approved controls, and existing audit contracts", () => {
    for (const file of GUARDED_FILES) {
      const source = readFileSync(file, "utf8");
  ...
  it("renders the empty new entry inside the new shell, never legacy UI", () => {
    ...
    expect(html).not.toContain("data-new-intake-placeholder");
    for (const token of BANNED_TOKENS) {
      expect(
        html.includes(token),
