/**
 * Answer shapes and pure transitions for the complete local intake journey.
 *
 ...
 * - `IntakeState`: the shell keeps a committed snapshot and a working copy
 *   of this shape. Screens stage changes; Lanjut commits through the
 *   preparation dependency boundary. Back restores committed answers.
 ...
 * - Fact-version comparison: the shell bumps the version when a committed
 *   change alters active meaning; the local question boundary rejects a
 *   pack made from a different version or input.
 *
 * Persistence belongs to local-session.ts; frozen question/start mapping
 * belongs to local-questions.ts. Neither reads or writes live workflow keys.
 */
...
/** Brand correction needs a name and an accepted public business source. */
export function isBrandFixValid(name: string, source = ""): boolean {
  return name.trim().length > 0 && parseSourceInput(source) !== null;
}
...
export function isMarketAnswerValid(
  ...
  if (kind === "sekitar") return areaCount === 1;
  if (kind === "beberapa") return areaCount > 0;
  return areaCount === 0;
}
...
export function setCategoryCustom(
  ...
  return {
    ...state,
    category: { selectedId: "category-custom-1", customLabel: label.trim() },
  };
}
...
export function toggleMarketArea(state: IntakeState, id: string): IntakeState {
  if (state.market.kind !== "sekitar" && state.market.kind !== "beberapa")
    return state;
  ...
  return {
    ...
    market: {
      ...state.market,
      areaIds,
      customAreas: single ? [] : state.market.customAreas,
    },
  };
}
...
export function addMarketArea(state: IntakeState, area: string): IntakeState {
  ...
  if (
    !area ||
    (state.market.kind !== "sekitar" && state.market.kind !== "beberapa")
  )
    return state;
  if (
    state.market.customAreas.some(
      (entry) =>
        entry.toLocaleLowerCase("id-ID") === area.toLocaleLowerCase("id-ID"),
    )
  )
    return state;
  const single = state.market.kind === "sekitar";
  return {
    ...
    market: {
      ...state.market,
      areaIds: single ? [] : state.market.areaIds,
      customAreas: single ? [area] : [...state.market.customAreas, area],
    },
  };
}
...
export function addCompetitor(state: IntakeState, name: string): IntakeState {
  ...
  if (
    !name ||
    state.competitors.custom.some(
      (entry) =>
        entry.toLocaleLowerCase("id-ID") === name.toLocaleLowerCase("id-ID"),
    )
  )
    return state;
  return {
...
export function isScreenAnswerValid(
  ...
  switch (screenId) {
    case "s-brand":
      ...
      return true;
    case "s-brand-fix":
      return isBrandFixValid(
        state.brandFixDraft.name,
        state.brandFixDraft.source,
      );
    case "s-scope":
      return state.scopeCommitted;
    case "s-branch":
      return (
        isSingleChoiceValid(state.branch.selectedId) &&
        !state.branch.custom.some(
          (item) =>
            item.id === state.branch.selectedId &&
            (!item.label.trim() || !item.detail?.trim()),
        )
      );
    case "s-product":
    ...
    case "s-competitors":
      return state.competitors.noDirect
        ? state.competitors.keptIds.length + state.competitors.custom.length ===
            0
        : state.competitors.keptIds.length +
            state.competitors.custom.filter((name) => name.trim()).length >
            0;
    default:
...
function singleLabel(
  ...
  const custom = answer.custom.find((item) => item.id === answer.selectedId);
  return custom
    ? `${custom.label}${custom.detail?.trim() ? ` — ${custom.detail.trim()}` : ""}`
...
export function deriveReviewRowsFromState(
  ...
  if (state.market.kind === null) {
    ...
    marketValue =
      all.length > 0
        ? `${state.market.kind === "sekitar" ? "Sekitar satu area" : "Beberapa area"} · ${all.join(", ")}`
        : "Belum dipilih";
  }
  ...
  if (
    (state.scope === "cabang" && active("s-branch")) ||
    (state.scope === "produk" && active("s-product"))
  ) {
    rows.push({
      key: "target",
      label: "Target audit",
  ...
  rows.push(
    ...
    ...(state.scope !== "produk" && active("s-offerings")
      ? [
          {
            key: "offerings",
...
export function summarizeCommittedAnswers(state: IntakeState): string {
  ...
  return JSON.stringify({
    ...
    branch:
      state.scope === "cabang"
        ? {
            ...
            custom: state.branch.custom
              .filter((item) => item.id === state.branch.selectedId)
              .map((item) => ({
    ...
    product:
      state.scope === "produk"
        ? {
            ...
            custom: state.product.custom
              .filter((item) => item.id === state.product.selectedId)
    ...
    offerings:
      state.scope !== "produk"
        ? {
            ...
            custom: state.offerings.custom
              .filter((item) => state.offerings.onIds.includes(item.id))
              .map((item) => item.label.trim())
              .sort(),
          }
        : null,
    customers: {
      ...
      custom: state.customers.custom
        .filter((item) => state.customers.onIds.includes(item.id))
        .map((item) => item.label.trim())
        .sort(),
    },
