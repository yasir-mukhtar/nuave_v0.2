...
describe("createIntakeState seeding (F1 rich)", () => {
  ...
  it("requires an explicit scope choice even when the fixture proposes one", () => {
    expect(state.scope).toBe("brand");
    expect(state.scopeOptionId).toBeNull();
...
describe("blocking validity from committed answers", () => {
  it("holds the F1 draft fully valid", () => {
    const state = setScopeAnswer(createIntakeState(F1), "scope-whole-brand");
    for (const id of [
      "s-brand",
      "s-scope",
      "s-category",
...
describe("review projection from committed state", () => {
  it("projects active state only, with optional empties and no inactive target", () => {
    for (const id of ["F1", "F2", "F3", "F4", "F5", "F6"] as const) {
      ...
      expect(fromState.some((row) => row.key === "target")).toBe(false);
      expect(fromState.find((row) => row.key === "scope")?.value).toBe(
        "Belum dipilih",
      );
      expect(fromState.find((row) => row.key === "facts")?.value).toBe(
        "Tidak ditambahkan",
      );
      expect(fromState.find((row) => row.key === "category")?.target).toBe(
        "s-category",
      );
    }
  });
  ...
  it("projects the committed single market area for F1", () => {
    ...
    expect(rows.find((row) => row.key === "market")?.value).toBe(
      "Sekitar satu area · Jakarta Selatan",
    );
  });
...
describe("remount regression: answers live outside the screen", () => {
  ...
  it("preserves committed answers across unmount and remount", () => {
    ...
    expect(renderMarket(shared)).toContain("Jakarta Selatan");
...
describe("selection regressions", () => {
  it("a deselected custom offering cannot satisfy the required selection", () => {
    ...
    state = toggleMultiAnswer(state, "offerings", "custom-1");
    expect(state.offerings.custom).toHaveLength(1);
    expect(isScreenAnswerValid("s-offerings", state)).toBe(false);
  });

  it("one-area selection is exclusive across prepared and custom areas", () => {
    let state = addMarketArea(createIntakeState(F1), "Depok");
    expect(state.market.areaIds).toEqual([]);
    expect(state.market.customAreas).toEqual(["Depok"]);
    state = toggleMarketArea(state, "city-2");
    expect(state.market.customAreas).toEqual([]);
    state = setMarketKind(state, "seluruh");
    expect(addMarketArea(state, "Bogor")).toEqual(state);
    expect(toggleMarketArea(state, "city-1")).toEqual(state);
  });

  it("no-direct mode clears names and a reselected name exits no-direct mode", () => {
    let state = setCompetitorsNoDirect(
      addCompetitor(createIntakeState(F1), "Kopi Baru"),
      true,
    );
    ...
    state = toggleCompetitor(state, "competitor-1");
    expect(state.competitors.noDirect).toBe(false);
    expect(state.competitors.keptIds).toEqual(["competitor-1"]);
  });

  it("custom categories keep a stable option id through replacement", () => {
    const state = setCategoryCustom(
      setCategoryCustom(createIntakeState(F1), "Kedai"),
      "Toko kopi",
    );
    expect(state.category).toEqual({
      selectedId: "category-custom-1",
      customLabel: "Toko kopi",
    });
  });
});
