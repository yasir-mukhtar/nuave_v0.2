...
export function isCategoryComparisonFallback(
  brief: MinimizedIndonesianBrief,
) {
...
export function hasIndonesianComparisonRelation(
  ...
  if (
    !brandIdentities.some((identity) =>
      containsIndonesianComparisonIdentity(text, identity),
    ) ||
    !containsIndonesianComparisonIdentity(text, comparisonIdentity)
  ) {
    return false;
  }
  ...
  const choicePairs = brandIdentities.flatMap((brandIdentity) => [
  ...
  return (
    markers.bracketed.every((marker) => tokens.includes(marker))
  );
}
...
export function validateCanonicalIndonesianQuestionPack(
  ...
  questions.forEach((question, index) => {
    ...
    const targetMentioned = Boolean(comparisonTarget) &&
