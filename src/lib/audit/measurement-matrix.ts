...
export const AUDIT_MEASUREMENT_MATRIX = [
  ...
  },
  {
    ...
    legacyAllowedContextFields: ["category", "market_context", "target_customer"],
  },
  {
  ...
  },
  {
    ...
    allowedContextFields: ["category", "market_context", "verified_decision_criteria"],
...
export type CanonicalMeasurementSlot = (typeof AUDIT_MEASUREMENT_MATRIX)[number];
...
export const PROMPT_MATRIX = AUDIT_MEASUREMENT_MATRIX.map(
  (slot) =>
    [slot.id, slot.legacyCategory, slot.legacyBranded, slot.legacyRole] as const,
