...
export async function POST(request: Request) {
  try {
    ...
    const promptIds = new Set(input.prompts.map((prompt) => prompt.prompt_id));
    ...
    for (const observation of resume) {
      ...
      if (!promptIds.has(observation.prompt_id)) {
        resumeErrors.push(
          `${observation.prompt_id}: resume observation is not in the locked question pack.`,
        );
      }
      ...
      async start(controller) {
        ...
        try {
          ...
          try {
            ...
            // The browser may have disconnected and cancelled the stream.
          }
        }
      },
      cancel() {
        // `request.signal` is owned by the runtime and is propagated through
        // orchestration/retry/provider work; no detached scheduling continues.
      },
    });
