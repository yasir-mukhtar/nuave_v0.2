...
import { customerAuditErrorMessage, type AuditCustomerStage } from "@/lib/audit/customer-error";
import {
...
export default function AuditWorkflow() {
  ...
  const [postReportBudgetCalls, setPostReportBudgetCalls] = useState<AuditCallTelemetry[]>([]);
  const [varianceRecord, setVarianceRecord] = useState<VarianceRecord | null>(null);
  const [varianceFailure, setVarianceFailure] = useState<VarianceFailureRecord | null>(null);
  ...
  const [reportFailureCode, setReportFailureCode] = useState<ReportFailureCode | null>(null);
  ...
  const reportCallCount = setupTelemetry.filter((call) => call.stage === "report").length;
  const reportRecovery = reportFailureCode
    ? classifyReportRecovery(reportFailureCode, reportCallCount, AUDIT_STAGE_CALL_LIMITS.report)
    : null;
  ...
  const runVariance = useCallback(
    async (
      ...
      const storedVariance = readStoredRunRecord<VarianceRecord>(VARIANCE_STORAGE_KEY);
      if (storedVariance?.run_key === runKey) {
        setVarianceRecord(storedVariance);
        setVarianceFailure(null);
        return;
      }
      const storedFailure = readStoredRunRecord<VarianceFailureRecord>(VARIANCE_FAILURE_STORAGE_KEY);
      if (storedFailure?.run_key === runKey) {
        setVarianceFailure(storedFailure);
        return;
      }

      const selectedPrompts = designatedVariancePrompts(promptPack.prompts, brief);
      ...
      if (proofErrors.length) {
        ...
        try {
          window.sessionStorage.setItem(VARIANCE_FAILURE_STORAGE_KEY, JSON.stringify(failure));
        } catch {
      ...
      try {
        const result = await postJson<{ variance: VarianceRecord; budget: AuditBudget }>(
          "/api/audit/variance",
          {
            brief,
        ...
        window.sessionStorage.setItem(VARIANCE_STORAGE_KEY, JSON.stringify(result.variance));
        ...
        if (isAbortError(cause) || !operationGeneration.isCurrent(operation)) return;
        const reason = cause instanceof Error ? cause.message : "Variance execution failed.";
        const failure = createVarianceFailureRecord({
          run_key: runKey,
          prompt_ids: selectedIds,
          incomplete_reason: reason,
    ...
    [brief, carryoverCostUsd, operationGeneration, promptPack, safetyIdentifier],
  );
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (saved) {
          ...
          setReportFailureCode(isReportFailureCode(state.reportFailureCode) ? state.reportFailureCode : null);
          setExecutionStarted(Boolean(state.executionStarted || restoredObservations.length));
          setPromptStatuses(initialStatuses(restoredPack, restoredObservations));
        }
        setVarianceRecord(readStoredRunRecord<VarianceRecord>(VARIANCE_STORAGE_KEY));
        setVarianceFailure(readStoredRunRecord<VarianceFailureRecord>(VARIANCE_FAILURE_STORAGE_KEY));
      } catch {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const response = await fetch("/api/audit/extract", { method: "GET", cache: "no-store" });
        const data = (await response.json()) as Partial<AuditBudget> & { error?: string };
        if (
          !response.ok ||
  ...
  const varianceSettled = Boolean(
    reportRunKey &&
      (varianceRecord?.run_key === reportRunKey || varianceFailure?.run_key === reportRunKey),
  );

  useEffect(() => {
    if (!restored || !budgetReady || busy || !report || !promptPack || varianceSettled) return;
  ...
  function updateBrief<K extends keyof BusinessBrief>(key: K, value: BusinessBrief[K]) {
  ...
  async function extractWebsite(url?: string) {
    ...
    setBusy("extract");
    try {
      const result = await postJson<{ draft: ExtractionDraft; telemetry: AuditCallTelemetry[] }>(
        "/api/audit/extract",
        {
      ...
      const similarBusinesses = sanitizeAiSimilarBusinesses(draft.similar_businesses ?? []);
      setExtraction({ ...draft, similar_businesses: similarBusinesses });
      setBrief((current) =>
        withPrimarySimilarBusiness({
          ...current,
          ...
          official_sources: [...new Set([resolvedUrl, ...draft.official_sources])],
  ...
  async function generatePrompts() {
    ...
    if (!factsConfirmed) {
      setError("Periksa fakta bisnis di atas dan konfirmasi sebelum melanjutkan.");
      return;
    }
    ...
    setBusy("prompts");
    try {
      const result = await postJson<{ pack: PromptPack; telemetry?: AuditCallTelemetry[] }>(
        "/api/audit/prompts",
        { brief: preparedBrief },
        operation.signal,
      );
  ...
  async function createReport(
    ...
    try {
      const reportInputCalls = dedupeAuditCalls([
        ...priorCalls,
        ...finalObservations.flatMap((observation) => observation.telemetry || []),
      ]);
      const reportResult = await postJson<{ report: AuditReport; telemetry?: AuditCallTelemetry[] }>(
        "/api/audit/report",
        {
          brief,
          prompts: promptPack.prompts,
      ...
      const varianceBudgetCalls = dedupeAuditCalls([...reportInputCalls, ...reportCalls]);
      ...
      setReportFailureCode(null);
      await runVariance(reportResult.report, varianceBudgetCalls, finalObservations);
    } catch (cause) {
      if (isAbortError(cause) || !operationGeneration.isCurrent(operation)) return;
      if (cause instanceof AuditRequestError && cause.telemetry.length) {
        setSetupTelemetry((calls) => dedupeAuditCalls([...calls, ...cause.telemetry]));
      }
  ...
  function handleRunEvent(event: AuditRunEvent, current: AuditObservation[]) {
    if (event.type === "prompt_started" || event.type === "attempt_started") {
      setPromptStatuses((statuses) => ({ ...statuses, [event.prompt_id]: "running" }));
    }
    if (event.type === "prompt_retrying") {
      setPromptStatuses((statuses) => ({ ...statuses, [event.prompt_id]: "retrying" }));
    }
    ...
    if (event.type === "prompt_failed") {
      setPromptStatuses((statuses) => ({ ...statuses, [event.prompt_id]: "failed" }));
    }
  ...
  async function runAudit() {
    ...
    if (questionErrors.length || blockers.length) {
      setError([...questionErrors.map((issue) => issue.message), ...blockers].join(" "));
      return;
    }

    const resumeObservations = observations.filter((observation) => observation.run_status === "completed");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new AuditRequestError(data.error || "Audit run failed.", [], data.code);
      }
      ...
      if (!runCompleted && !runUnfinishedReceived) throw new Error("Audit stream ended early.");
      if (runCompleted && finalObservations.length !== 10) throw new Error("Invalid completed observation set.");
      ...
      if (!isAbortError(cause) && (handedToReport || operationGeneration.isCurrent(operation))) {
        setError(safeError(handedToReport ? "report" : "run", cause));
      }
    } finally {
      if (!handedToReport && operationGeneration.isCurrent(operation)) setBusy(null);
      operationGeneration.finish(operation);
    }
  }
  ...
  function handleLogo(file: File | undefined) {
    ...
    reader.onload = () => updateBrief("agency_logo_data_url", String(reader.result || ""));
    reader.readAsDataURL(file);
  }

  function downloadEvidenceJson() {
    if (!report || !promptPack) return;
    const evidence = makeEvidenceExport(brief, promptPack.prompts, observations, report);
    const blob = new Blob([JSON.stringify(evidence, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    const brandSlug =
      brief.brand_name.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") ||
      "client";
    link.href = url;
    ...
    <main className={styles.shell} lang="en" data-theme="light">
      ...
      >
        <Link href="/" className={styles.brand}>
          <Image src="/logo-nuave-horizontal.png" width={152} height={48} priority alt="Nuave" />
        </Link>
        <div className={styles.topActions}>
          <Button variant="secondary" size="sm" onPress={startOver}>Mulai ulang</Button>
      ...
      {step > 0 && step < 4 ? (
        <nav className={`${styles.stepper} ${styles.noPrint}`} aria-label="Tahapan audit">
          <div className={styles.stepContext}>
            <span>Pengaturan audit</span>
            <strong>Langkah {step + 1} dari {stepLabels.length}</strong>
          </div>
      ...
      {step === 0 || exiting ? (
        <SourceHero initialValue={websiteUrl} extracting={busy === "extract"} error={error} onExtract={extractWebsite} exiting={exiting} />
      ) : null}
      ...
      {step === 2 && promptPack ? (
        <QuestionsStep pack={promptPack} brandName={brief.brand_name} busy={busy} onEdit={editPrompt} onBack={backToFacts} onRun={runAudit} />
      ) : null}
      ...
      {step === 4 && report && promptPack ? (
        <ReportView report={report} brief={brief} observations={observations} onDownloadJson={downloadEvidenceJson} />
      ) : null}
    </main>
  );
}
