...
export default function ActionItemPanel({ items, auditId, maxListHeight }: Readonly<ActionItemPanelProps>) {
  ...
  return (
    <div className="border border-border-light rounded-[var(--radius-sm)] shadow-app-subtle bg-white flex flex-col overflow-hidden h-full">
      ...
      <div className="flex items-center justify-between h-14 shrink-0 px-5">
        ...
        {auditId && (
          <span
            className="type-body flex items-center gap-1 px-3 py-1.5 font-[var(--btn-font-weight)] text-text-heading border border-border-light rounded-[var(--radius-sm)] shadow-app-subtle hover:border-[var(--border-default)] hover:bg-gray-50 transition-colors cursor-pointer select-none"
          >
            {t('cta.viewAll')}
