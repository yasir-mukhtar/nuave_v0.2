...
| --- | --- | --- | --- |
| G0 | Complete | `ab8336b4b9a79051b1ce9b4c7cfe13156bf45d92` (R5 adoption + superseded-plan retirement + routing) merged to `main` via PR #48 (`c5fdb74`); post-merge baseline recorded above on `main@f54e805` (PRs #15/#27/#48/#49 merged; #46 closed unmerged; #47 open/parked); live bundle `13c4836d` → `main@f54e805`, zero bound secrets (Cloudflare API 2026-09-12); founder-authorized live check 2026-09-12 — generation does NOT succeed, deterministic fallback serves (above); `fixture-kopi-taman-senja.ts` (`NVA-FIKTIF-001.questions.v1`) intact as the preserved ordinary pack; `docs/AUDIT.md`, `docs/PROMPT_GENERATION_CONTEXT.md`, `docs/journey/04-questions.md`, `docs/INDEX.md`, `docs/NOW.md`, `specs/README.md`, and the `SPEC.md` intake pointer reconciled to R5 with no conflicting authority | G1 is dependency-ready — map confirmed context and correction targets against `src/app/audit/intake/` → `POST /api/audit/prompts`. Live generation failing to fallback is a recorded operational fact for the G2P/provider feasibility work, not a G1 blocker. |
| G1 | Complete — uncommitted working tree on `feat/spec-008-g1-facts-context` (base `main@505ccd4`) | Dormant §3.1/§3.2/§6.1 module `src/lib/audit/questions-id-v3-context.ts` (new; imported only by its test — no live-path wiring): `projectV3ConfirmedFacts` pure adapter (separate `entityScope`, conservative `entityType`, typed `buyerConstraints` with access/fulfilment subset, structured `marketContext`, `serviceChannels`, confirmed comparator, identity registry, target signals held local), `effectiveV3Context(slot)` = legacy `allowedContextFields` ∪ shared (scope/market/channels/access-fulfilment constraints) ∪ slot-3 `customerNeeds` addition with §3.2 denials recorded, `v3FactsFingerprint` SHA-256 over canonical adapter output, `v3FactsDeficiencies` + `classifyPromptsBriefParseFailure` mapping `INPUT_CORRECTION_REQUIRED` to existing `IntakeScreen` destinations used by both renderers. Optional facts seam: `service_areas`/`service_channels` optional on `businessBriefSchema` (+ `FIELD_OWNERSHIP`/`parseWorkflowStorageState` entries); `measurement-matrix.ts` untouched; `INDONESIAN_PRIVATE_DATA_PATTERNS` exported for reuse. Fixtures `questions-id-v3-context.test.ts` (26 tests): scope kinds, slot 2 Depok/home-visit, slot 3 approved need, absent channels absent, identity on slots 7–10/comparator on 9 only, no URLs/free text/target signals in context, sensitive customer facts dropped, fingerprint invalidation, conflict/absence deficiencies, malformed-vs-correctable parse split. `npm run verify` green offline (typecheck/lint/format/typography, 862 unit tests, builds, e2e). | G2 is dependency-ready per §7 ordering: freeze evaluation rules, partitions, rubric, configurations, budgets, ceilings, and §8.3 counterexamples; use recorded G0 provider config (opencodego/`gpt-5.6-luna`, low effort, 2048-token cap, session headers, no search). Provider feasibility remains the recorded G2P question — do not assume it at G2. |
