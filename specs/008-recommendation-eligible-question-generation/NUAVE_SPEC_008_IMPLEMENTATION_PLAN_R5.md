...
| G1 | Implemented and offline-verified locally; not landed | `codex/spec008-g1-adapter` over refreshed `origin/main@505ccd4`; dormant adapter/context map and 44 parsing/projection tests; accepted intake snapshots from `5821d2f`. Full offline verify passed: 880 unit tests, 84 browser tests, both builds. See `VERIFICATION.md`. Founder authorized commit/push after verification; no merge, route integration or provider call. | Review the G1 branch, then G2 evaluation freeze; resolve explicit unknown role and multi-comparator relation before affected integration. |
