...
owns the newly approved recommendation-eligible intake-to-report delivery.
R5's rich writer, per-slot contracts, P/M/C experiments and remaining pilot/
release gates are historical for that work, not prerequisites to continue it.
This does not change frozen results, consumed allowances or production state.
Keep applicable evidence/privacy protections; new paid scopes need authorization.
...
| G1 | Implemented, independently reviewed, and prepared for integration; still dormant and unmerged | Draft PR #59 `codex/spec008-g1-adapter`, merged onto `main@ac3cc50` on 2026-09-13; dormant adapter/context map with three-scope serialized fixture, 59 adapter tests, 6 retained intake-serializer compatibility checks, and 2 port cases; independent review 2026-09-13 found no application defect and verified real `freezeLocalIntake` parity; full offline verify passed at the published head (totals in `VERIFICATION.md`). No merge, route integration, provider call, or activation is claimed. | Founder's integration decision on #59, then G2 evaluation freeze. Role stays `factsContext`-sourced; undesignated comparators keep the recorded category-alternatives policy. |
