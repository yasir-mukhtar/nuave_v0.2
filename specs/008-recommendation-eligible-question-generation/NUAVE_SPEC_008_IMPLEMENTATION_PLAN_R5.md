...
| --- | --- |
| Canonical path / adopted revision commit | `specs/008-recommendation-eligible-question-generation/NUAVE_SPEC_008_IMPLEMENTATION_PLAN_R5.md`, adopted at `ab8336b4b9a79051b1ce9b4c7cfe13156bf45d92` on branch `docs/spec-008-recommendation-eligible-questions` over `main@e531ff4653c324007eb049bee93f2a3b922cf216` (unchanged since Review 5's inspection). Review 5 artifact committed beside this file at `NUAVE_SPEC_008_ADVERSARIAL_REVIEW_5.md`; provenance hashes in `VERIFICATION.md`. Adoption merged to `main` via PR #48 (merge `c5fdb74`, 2026-09-11T23:26Z); G0 baseline reconciliation continued on branch `docs/spec-008-g0-baseline-reconcile` over `main@f54e805`. |
| Actual runtime/provider, intake handoff, old bundle and recovery path | Repo evidence at baseline: question writer `NUAVE_QUESTION_PROVIDER=opencodego` (production-pinned; openai/gemini/openrouter testing-only), `gpt-5.6-luna` via `OPENAI_AUDIT_MODEL`/`AUDIT_MODEL`, endpoint `https://opencode.ai/zen/go/v1`, reasoning `low`, `INDONESIAN_QUESTION_MAX_OUTPUT_TOKENS=2048`, no search (one bounded call + deterministic Indonesian fallback). Intake handoff: `BusinessBrief` JSON to `POST /api/audit/prompts`; `src/lib/intake/` does not exist at baseline — PR #46 `feat/airbnb-intake-rebuild` remains OPEN at `afd518dd75d436319c7a5f1c31db9d640e2728d3` with question wiring still unowned by intake; PR #47 (cheaper-inference experiment) also open, neither merged into this baseline. Deployed/old client bundle: partially verified via Cloudflare API 2026-09-11 — worker `nuave-v2` serves `v2.nuave.ai` (production); live deployment `bcd725e5-1916-4e70-84c0-65491228960a` → version `a4540241-08e2-4612-bbc2-d0a89905bfed` at 100%, deployed 2026-09-02T10:09Z — 7 minutes after the `e531ff4` merge via the merged-PR deploy gate, so the live bundle maps to `main@e531ff4653c324007eb049bee93f2a3b922cf216`, the same commit R5/Review 5 inspected (predates PRs #46/#47 and this adoption); compat `nodejs_compat`, date 2026-08-01; bindings: `ASSETS` + three rate limiters only — **zero secret/plain-text bindings**, so served provider config is code defaults (no `OPENCODEGO_API_KEY`/`NUAVE_QUESTION_PROVIDER` bound at platform layer; whether live generation on v2.nuave.ai currently succeeds was an open operational question — resolved 2026-09-12 below: it does not; the deterministic fallback serves). The actual unprepared-bundle rehearsal against this deployed build remains a G5 verification item. Recovery path: pre-start fact correction regenerates the pack under the same order; post-start runs lock; state persists in browser `sessionStorage` only. **Post-merge reconciliation, 2026-09-12 (`main@f54e805`):** PR #48 merged this adoption; PR #49 merged Spec 007 R-27 intake recovery (`7a2e0e7`), adding `src/app/audit/intake/` (IntakeShell plus scope/branch/offerings/review/questions screens and `questionEditTransaction`) and `src/components/product/selection/`; `src/lib/intake/` never landed and PR #46 closed unmerged 2026-09-11T23:32Z; PR #47 remains open and parked. Intake handoff unchanged: `BusinessBrief` JSON → `POST /api/audit/prompts` via `AuditWorkflow.generatePrompts`. The new intake module owns question review/edit presentation only on the preview route `/audit/v2/intake-preview` — `PRODUCTION_INTAKE_SURFACE` is empty, so production still serves the existing renderer — while generation dispatch stays in `AuditWorkflow`, not the intake module. Question-boundary code diff since `e531ff4`: one Indonesian duplicate-question error message only (`questions-id.ts`); provider config, prompts/run routes, lock boundary, and matrix unchanged. Re-verified via Cloudflare API 2026-09-12: live version `13c4836d-70a6-4313-92b7-18ef57713c90` at 100%, deployed 2026-09-11T23:57Z (~7 minutes after the `f54e805` merge) → live bundle maps to `main@f54e805`; bindings still `ASSETS` + three rate limiters with zero secret/plain-text bindings. Founder-authorized live check 2026-09-12: one `POST /api/audit/prompts` on `v2.nuave.ai` with the fictional Kopi Taman Senja brief returned HTTP 200 in ~0.8s with `generation.source: "fallback"` and `warnings: ["fallback_used"]` — the provider attempt failed (`provider_status: 400`, 0 tokens, USD 0, `preflight_reservation` cost basis, "no model provenance") and the route served the deterministic Indonesian pack (10 questions, 6/4 classification, `question-writer-v2`). Live model-written generation therefore did NOT succeed on v2.nuave.ai at that revision; the deterministic fallback served in its place. A first request carrying object-shaped `official_sources` bounced on Zod validation (400, no provider call); the corrected string form produced the attempt above. **Resolved 2026-09-12:** the 400 was `MissingSessionID` — OpenCode Go requires `x-opencode-session` for routing. PR #51 (`ce979e1`) added the session header and client `User-Agent` on all OpenCode Go transports; after deploy (version `c0fa5fd1`→ successor on `main@505ccd4`), the same fictional-brief check returned `generation.source: "model"`, `gpt-5.6-luna`, `resp_03de…`, 1,381 tokens, USD 0.0006, no fallback. |
...
| G2 input partitions, rules, configuration hashes, numeric absolute/incremental ceilings and authorization | Frozen offline on `codex/spec008-g2-evaluation-freeze`, corrected 2026-09-13 after the independent F1–F10 review, corrected again the same day after the independent R1–R6 re-review, and corrected a third time after the independent second re-review, a fourth time after the independent third re-review, and a fifth time after the independent fourth re-review: packet `specs/008-recommendation-eligible-question-generation/G2_EVALUATION_PACKET.md` (`nuave.g2-evaluation-packet.v6`, decision `nuave.g2-decision-policy.v6`, rubric `nuave.g2-review-rubric.v2`, frozen inputs `nuave.g2-frozen-inputs.v2`, usage accounting `nuave.g2-usage-accounting.v2`) with executable decision policy in `src/lib/audit/question-eval-g2.ts`. Frozen inputs: 12 committed fictional envelopes in `src/lib/audit/fixtures/g2-evaluation-inputs.json` bound by a SHA-256 manifest with pinned per-input `envelopeSha256` cross-checked by `validateG2FrozenInputs`; the v1 held-out envelopes were retired from untouched held-out status after their deterministic fallback forms were rendered/inspected during correction and re-review (retirement record + retired fingerprints in `G2_FROZEN_INPUT_MANIFEST.retiredInputs`), and v2 freezes fresh envelopes of the same four approved archetypes with no fallback/model output generated or inspected. §8.1: G2P-AC/RETAIL/B2B/SPARSE alias D1/D3/D5/D8 + predeclared AC repeat, 5 rich + 5 simple ≤10 calls. §8.2: D1–D8 dev, H1–H4 held out, repeats D1/D3/D5/H1; rich path 16 v3 + 11 v2 + 5 simple ≤32 new calls. Comparison: per matched input+pass pair over six unnamed texts — material win = usable rich plus unusable simple or ≥0.5 unnamed-mean advantage; ≥2 distinct businesses, business counted once, any pair-level regression fails the applicable decision; missing pack scores zero; ties favor simple; one global selection policy (default/M). Release evaluation: all 16 selected-v3 packs usable, per-input all-ten-text naturalness non-worse vs actual v2 (11 actual-v2 attempts across the eight development inputs under rich selection, all 16 under simple-by-amendment), §8.2.5 integrated pilot-replay and release-attribution gates mandatory. Resources from complete attempt records: per-attempt 4,096-token/60 s/$0.02 usage ceilings; mean-generation-cost ceiling $0.02 set equal to the per-attempt bound; totals $0.20 pilot / $0.64 release at dated 2026-09-13 OpenCode Go usage-allowance rates ($0.20/$0.02/$0.25/$1.20 per 1M, subscription allowance not cash) with cached-read/cached-write input portions priced separately and validated nonnegative integer counters; mean latency 30 s, nearest-rank p95 60 s; held-out increments ≤2.0× cost / ≤1.5× latency; retries 0, concurrency 1. Transport contract pins the accepted OpenCode Go method (Authorization bearer, `application/json`, fresh `x-opencode-session` per call, `User-Agent: nuave-audit/1.0` via the code-owned `opencodeGoTransportHeaders` helper). Content hashes in `VERIFICATION.md` and the correction evidence. Awaits independent G2 acceptance and a separately authorized G2P; no provider calls or spend authorized. |
...
| --- | --- | --- | --- |
| G0 | Complete | `ab8336b4b9a79051b1ce9b4c7cfe13156bf45d92` (R5 adoption + superseded-plan retirement + routing) merged to `main` via PR #48 (`c5fdb74`); post-merge baseline recorded above on `main@f54e805` (PRs #15/#27/#48/#49 merged; #46 closed unmerged; #47 open/parked); live bundle `13c4836d` → `main@f54e805`, zero bound secrets (Cloudflare API 2026-09-12); founder-authorized live check 2026-09-12 — generation does NOT succeed, deterministic fallback serves (above); `fixture-kopi-taman-senja.ts` (`NVA-FIKTIF-001.questions.v1`) intact as the preserved ordinary pack; `docs/AUDIT.md`, `docs/PROMPT_GENERATION_CONTEXT.md`, `docs/journey/04-questions.md`, `docs/INDEX.md`, `docs/NOW.md`, `specs/README.md`, and the `SPEC.md` intake pointer reconciled to R5 with no conflicting authority | G1 is dependency-ready — map confirmed context and correction targets against `src/app/audit/intake/` → `POST /api/audit/prompts`. Live generation verified working post-PR #51 (above); provider feasibility evidence begins at G2P under its own authorization. |
| G1 | **Merged and deployed; still dormant** | Merged to `main` via PR #59 merge commit `687f340343aa5be547d03e7d6fe23a9f5262a2df` (2026-09-13), tree `1e0bae2f…` identical to the offline-verified preparation tree; main-push CI + Cloudflare deploy green (worker `nuave-v2` version `a2279ec7-109d-4d43-8ecd-026cc669e8a0`); remote branch auto-deleted, local branch/worktree preserved. Adapter/context map with three-scope serialized fixture, 59 adapter tests, 6 retained intake-serializer checks, 2 port cases. No runtime consumer — deployed but dormant. | G2 evaluation freeze (this row's gate), then a separately authorized G2P pilot. |
| G2 | **Fifth correction offline-verified; awaiting independent re-review/acceptance** | Branch `codex/spec008-g2-evaluation-freeze` over `main@687f340`; dormant prototype `question-writer-v3.ts`/`question-finalize-v3.ts` + frozen packet `question-eval-g2.ts` and `G2_EVALUATION_PACKET.md` corrected against the independent F1–F10 review, the independent R1–R6 re-review, the independent second re-review, the independent third re-review (restored R5 §8.1 OR-component rule, replay/capture validation and derived counters, coherent D2/D6 fallback decisions, corrected conflict-index mapping), and now the independent fourth re-review (source-response fingerprints bind each replay to the exact parsed response — an identical M selection no longer proves the same source — and required component gates derive from the adopted reserve-bearing contract rather than caller flags); fresh v2 held-out envelopes remain frozen and unexposed; executable §8.3 counterexamples and regression tests; full offline `npm run verify` recorded in `VERIFICATION.md`. Not an empirical quality pass; provider feasibility unproven; no provider calls, merge, or activation. | Independent orchestrator re-review of the fifth corrected prototype, rubric, fallback examples, and frozen rules; then a separately authorized G2P pilot. |
