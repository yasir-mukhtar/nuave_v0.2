...
| G2 | **Second correction offline-verified; awaiting independent re-review/acceptance** | Branch `codex/spec008-g2-evaluation-freeze` over `main@687f340`; dormant prototype `question-writer-v3.ts`/`question-finalize-v3.ts` + frozen packet `question-eval-g2.ts` and `G2_EVALUATION_PACKET.md` corrected against the independent F1–F10 review, then corrected again against the independent R1–R6 re-review (accepted transport contract, exact fingerprint/policy/version bindings, honest release evaluator with all-16-usable and per-input v2 naturalness gates, cached-token usage accounting and resource samples, purpose-preserving fallback forms on the shared `safetyRestrictions` field, bounded named-slot backtracking with true affected-slot detection); fresh v2 held-out envelopes replace the exposed v1 set (retirement recorded in the manifest); executable §8.3 counterexamples and regression tests; full offline `npm run verify` recorded in `VERIFICATION.md`. Not an empirical quality pass; provider feasibility unproven; no provider calls, merge, or activation. | Independent orchestrator re-review of the second corrected prototype, rubric, fallback examples, and frozen rules; then a separately authorized G2P pilot. |
