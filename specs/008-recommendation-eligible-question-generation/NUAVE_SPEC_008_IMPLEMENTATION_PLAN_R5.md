...
| G2 input partitions, rules, configuration hashes, numeric absolute/incremental ceilings and authorization | Frozen offline on `codex/spec008-g2-evaluation-freeze`, corrected 2026-09-13 after the independent F1–F10 review: packet `specs/008-recommendation-eligible-question-generation/G2_EVALUATION_PACKET.md` (`nuave.g2-evaluation-packet.v2`, decision `nuave.g2-decision-policy.v2`, rubric `nuave.g2-review-rubric.v2`, frozen inputs `nuave.g2-frozen-inputs.v1`, usage accounting `nuave.g2-usage-accounting.v1`) with executable decision policy in `src/lib/audit/question-eval-g2.ts`. Frozen inputs: 12 committed fictional envelopes in `src/lib/audit/fixtures/g2-evaluation-inputs.json` bound by SHA-256 manifest. §8.1: G2P-AC/RETAIL/B2B/SPARSE alias D1/D3/D5/D8 + predeclared AC repeat, 5 rich + 5 simple ≤10 calls. §8.2: D1–D8 dev, H1–H4 held out, repeats D1/D3/D5/H1; rich path 16 v3 + 11 v2 + 5 simple ≤32 new calls. Comparison: per matched input+pass pair over six unnamed texts — material win = usable rich plus unusable simple or ≥0.5 unnamed-mean advantage; ≥2 distinct businesses, business counted once, any pair-level regression fails the applicable decision; missing pack scores zero; ties favor simple; one global selection policy (default/M). Resources from complete attempt records: per-attempt 4,096-token/60 s/$0.02 usage ceilings; totals $0.20 pilot / $0.64 release at dated 2026-09-13 OpenCode Go usage-allowance rates ($0.20/$0.02/$0.25/$1.20 per 1M, subscription allowance not cash); mean latency 30 s, nearest-rank p95 60 s; held-out increments ≤2.0× cost / ≤1.5× latency; retries 0, concurrency 1. Content hashes in `VERIFICATION.md` and the correction evidence. Awaits independent G2 acceptance and a separately authorized G2P; no provider calls or spend authorized. |
...
| G2 | **Corrected and offline-verified; awaiting independent re-review/acceptance** | Branch `codex/spec008-g2-evaluation-freeze` over `main@687f340`; dormant prototype `question-writer-v3.ts`/`question-finalize-v3.ts` + frozen packet `question-eval-g2.ts` and `G2_EVALUATION_PACKET.md` corrected against the independent F1–F10 review (comparison rule, complete evidence, resource accounting, executable input freeze, strict schema, components, recovery, guard pairs, fallback forms); executable §8.3 counterexamples and regression tests; full offline `npm run verify` on the published head (counts in `VERIFICATION.md`). Not an empirical quality pass; provider feasibility unproven; no provider calls, merge, or activation. | Independent orchestrator re-review of the corrected prototype, rubric, fallback examples, and frozen rules; then a separately authorized G2P pilot. |
