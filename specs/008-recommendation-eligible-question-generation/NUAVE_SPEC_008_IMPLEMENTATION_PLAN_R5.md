...
| G2 input partitions, rules, configuration hashes, numeric absolute/incremental ceilings and authorization | Frozen offline on `codex/spec008-g2-evaluation-freeze` (2026-09-13): packet `specs/008-recommendation-eligible-question-generation/G2_EVALUATION_PACKET.md` (`nuave.g2-evaluation-packet.v1`) with executable decision policy in `src/lib/audit/question-eval-g2.ts`. §8.1: G2P-AC/RETAIL/B2B/SPARSE + predeclared AC repeat, 5 rich + 5 simple ≤10 calls. §8.2: D1–D8 dev, H1–H4 held out, repeats D1/D3/D5/H1; rich path 16 v3 + 11 v2 + 5 simple ≤32 new calls. Comparison: pooled per-business naturalness delta ≥0.3 material win, ≥2 distinct businesses, no pair regressions; rich ≤2.0× simple mean cost, ≤1.5× latency; per-attempt 4,096-token/60 s/$0.20 ceilings; totals $2.50 pilot / $7.00 release (assumed $25/1M blended, re-check invoice basis at G2P). Content hashes in `VERIFICATION.md` and the G2 report evidence. Awaits independent G2 acceptance and a separately authorized G2P; no provider calls or spend authorized. |
...
| G2 | **Prepared and offline-verified; awaiting independent acceptance** | Branch `codex/spec008-g2-evaluation-freeze` over `main@687f340`; dormant prototype `question-writer-v3.ts`/`question-finalize-v3.ts` + frozen packet `question-eval-g2.ts` and `G2_EVALUATION_PACKET.md`; executable §8.3 counterexamples and prototype mechanics tests; full offline `npm run verify` on the published head (counts in `VERIFICATION.md`). Not an empirical quality pass; provider feasibility unproven; no provider calls, merge, or activation. | Independent orchestrator review of the prototype, rubric, fallback examples, and frozen rules; then a separately authorized G2P pilot. |
