...
| G2 | **Third correction offline-verified; awaiting independent re-review/acceptance** | Branch `codex/spec008-g2-evaluation-freeze` over `main@687f340`; dormant prototype `question-writer-v3.ts`/`question-finalize-v3.ts` + frozen packet `question-eval-g2.ts` and `G2_EVALUATION_PACKET.md` corrected against the independent F1–F10 review, the independent R1–R6 re-review, and now the independent second re-review (exact variant-scoped attribution binding to captured P/M/C texts/origins/fingerprints, canonical request-configuration fingerprints on attempt records, an explicit `retain`/`amendment_required`/`not_retained` combined outcome, a purpose-based fallback contract with no keyword gate, and matching-based multi-slot conflict repair preserving the two-pass budget); fresh v2 held-out envelopes remain frozen and unexposed; executable §8.3 counterexamples and regression tests; full offline `npm run verify` recorded in `VERIFICATION.md`. Not an empirical quality pass; provider feasibility unproven; no provider calls, merge, or activation. | Independent orchestrator re-review of the third corrected prototype, rubric, fallback examples, and frozen rules; then a separately authorized G2P pilot. |
